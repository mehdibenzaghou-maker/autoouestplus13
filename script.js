...
// ---------- Build car showcase blocks ----------
function renderCards(){
  carList.innerHTML = CARS.map(car => `
    <article class="car-block" data-id="${car.id}" tabindex="0" role="button" aria-label="Voir la fiche ${car.name}">
      <div class="car-block-media">
        <span class="car-block-tag">${car.tag}</span>
        <img src="${car.coverPhoto}" alt="${car.name}" loading="lazy">
      </div>
      <div class="car-block-text">
        <div class="car-block-name">${car.name}</div>
        <div class="car-block-year">${car.year}</div>
        <div class="car-block-specs">
          ${car.quickSpecs.map(s => `<div><strong>${s.value}</strong>${s.label}</div>`).join("")}
        </div>
        <div class="car-block-cta">Voir la fiche complète →</div>
      </div>
    </article>
  `).join("");

  carList.querySelectorAll(".car-block").forEach(card => {
